...
bins<-read.csv("/Users/elizabeth.chin/Desktop/milk/FL100_FINAL_Bins_agesexbmi_clean.csv", header=T)

#IDs of subjects w/ sequencing data
seqdata<- read.csv("/Users/elizabeth.chin/Desktop/milk/16S/SubjectIDs_2019-12-19.csv", header=T)
...
sum(is.na(inf.lac$Lactose)) #0 noice
sum(is.na(inf.lac$FoodAmt)) #54
noamt<- inf.lac %>% filter(is.na(FoodAmt))
...
```{r}
inf.lac2<- inf.lac %>% select(UserName, RecallNo, FoodCode.x, Food_Description.x, FoodAmt, Lactose, NCC.Food.Description, D_TOTAL)
items.lac2<- items.lac %>% select(UserName, RecallNo, FoodCode.x, Food_Description.x, FoodAmt, Lactose, NCC.Food.Description, D_TOTAL)

all.lac<- rbind(inf.lac2, items.lac2)
colnames(all.lac)<-c("SubjectID", "RecallNo", "FoodCode", "Food_Description", "FoodAmt", "Lactose.p100g", "NCC.Food.Description", "D_TOTAL")
...
```{r}
per.recall<- aggregate(cbind(Lactose.consumed, D_TOTAL) ~ SubjectID + RecallNo, data=qc.lac, FUN="sum")
dim(per.recall) #1101
#check
...
write.csv(per.subject, "/Users/elizabeth.chin/Desktop/milk/FL100_lactose_lct_16s/data/01_asa24_lactose_dtotal.csv", row.names=F)
write.csv(per.recall, "/Users/elizabeth.chin/Desktop/milk/FL100_lactose_lct_16s/data/01b_asa24_lactose_dtotal_per_recall.csv", row.names=F)
...
write.csv(nococoa, "/Users/elizabeth.chin/Desktop/milk/FL100_lactose_lct_16s/data/02_asa24_lactose_dtotal_no_powders.csv", row.names=F)
...
```{r}
master<- merge(merge(lct, eth, by = "SubjectID"), bins, by.x = "SubjectID", by.y="subject_id") #349 subjects
